...
@register_logic(
    name="039_missing_guardian_cnic_for_minor",
    description="Flags minor customers (sector code 1006) whose guardian CNIC is missing in KYC profile.",
    category="Compliance & Screening",
)
def logic_039_missing_guardian_cnic_for_minor(
    ...
    try:
        ...
        guardian_id_col = (
            "GUARDIAN_ID_NUMBER2" if "GUARDIAN_ID_NUMBER2" in df_kyc.columns else None
        )
        guardian_name_col = (
        ...
        if guardian_id_col:
            df_kyc["GUARDIAN_ID_NUMBER2_CLEAN"] = (
                df_kyc[guardian_id_col].fillna("").astype(str).str.strip().str.lower()
            )
        else:
            df_kyc["GUARDIAN_ID_NUMBER2_CLEAN"] = ""

        # Flag minors with missing guardian CNIC
        df_flagged = df_kyc[
        ...
        output_cols = [
            ...
            guardian_id_col if guardian_id_col else None,
            "PRODUCTDESC",
            "SECTOR_DESCRIPTION",
        ]
        output_cols = [
            col for col in output_cols if col
        ]  # drop None if guardian fields missing
        ...
        if guardian_id_col:
            rename_map[guardian_id_col] = "Guardian_CNIC"
        ...
        if mode == "full":
            file_path = (
                f"missing_guardian_cnic_for_minor_{datetime.now():%Y%m%d_%H%M%S}.xlsx"
            )
        ...
        print(f"❌ Error in logic_039_missing_guardian_cnic_for_minor: {e}")
        return pd.DataFrame(
            [
                {
