...
@register_logic(
    name="057_salary_other_income_missing_for_expected_profiles",
    description="Flags salaried, retired, housewife, and student profiles where Salary_Other_Income is missing.",
    category="Compliance & Screening",
)
def logic_057_salary_other_income_missing_for_expected_profiles(
    ...
    try:
        ...
        # Add hardcoded null-like values
        raw_nill_values.update(
            ["0", "0.0", "NA", "N/A", "NONE", "NULL", "-", "--", "?", ""]
        )
        ...
        # 🧠 Exclusion list
        exclusion_set = {"BUSINESS", "LANDLORD", "OTHERS", "MINOR"}
        ...
        df_filtered = df[
            ...
            & (~df["CUSTOMER_OCCUPATION_NORMALIZED"].isin(exclusion_set))
        ].copy()
        ...
        if mode == "full":
            file_path = (
                f"salary_other_income_missing_{datetime.now():%Y%m%d_%H%M%S}.xlsx"
            )
