...
@register_logic(
    name="054_employer_name_fed_for_non_salaried_individuals",
    description="Flags non-salaried individuals where NameOfEmployer is inappropriately fed.",
    category="Compliance & Screening",
)
def logic_054_employer_name_fed_for_non_salaried_individuals(
    ...
    try:
        ...
        nill_set_normalized = {normalize_field(val) for val in raw_nill_values}
        ...
        df_filtered = df[
            (~df["NAMEOFEMPLOYER_NORMALIZED"].isin(nill_set_normalized))
        ...
        # 📁 Optional export
        if mode == "full":
            file_path = f"employer_name_fed_for_non_salaried_{datetime.now():%Y%m%d_%H%M%S}.xlsx"
