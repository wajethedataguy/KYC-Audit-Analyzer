...
@register_logic(
    name="053_missing_employment_status_for_salaried_individuals",
    description="Flags salaried individuals with missing or vague EmploymentStatus using Nill_Comb reference list.",
    category="Compliance & Screening",
)
def logic_053_missing_employment_status_for_salaried_individuals(
    ...
    try:
        ...
        nill_set_normalized = {normalize_status(val) for val in raw_nill_values}
        ...
        df["EMPLOYMENTSTATUS_CLEAN"] = df["EMPLOYMENTSTATUS"].astype(str).str.strip()
        df["EMPLOYMENTSTATUS_NORMALIZED"] = df["EMPLOYMENTSTATUS_CLEAN"].apply(
            normalize_status
        )
        ...
        df_filtered = df[
            (
                (df["EMPLOYMENTSTATUS_CLEAN"] == "")
                | (df["EMPLOYMENTSTATUS_CLEAN"].isna())
                | (df["EMPLOYMENTSTATUS_NORMALIZED"].isin(nill_set_normalized))
            )
        ...
        # 📁 Optional export
        if mode == "full":
            file_path = f"missing_employment_status_{datetime.now():%Y%m%d_%H%M%S}.xlsx"
