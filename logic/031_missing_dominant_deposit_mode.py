...
@register_logic(
    name="031_missing_dominant_deposit_mode",
    description="Flags customers whose Dominant Mode of Deposit is blank in KYC profile.",
    category="Compliance & Screening",
)
def logic_031_missing_dominant_deposit_mode(
    ...
    try:
        ...
        df["DOMINANT_MODE_CLEAN"] = (
            df["DOMINANT_MODE_OF_DEPOSIT"].astype(str).str.strip()
        )

        # Treat as missing if:
        #   - original value is NaN OR
        #   - stripped text is empty
        mask_missing = df["DOMINANT_MODE_OF_DEPOSIT"].isna() | (
            df["DOMINANT_MODE_CLEAN"] == ""
        )
        ...
        if mode == "full":
            file_path = (
                f"missing_dominant_deposit_mode_{datetime.now():%Y%m%d_%H%M%S}.xlsx"
            )
        ...
        print(f"❌ Exception occurred in logic: {e}")
        fallback_columns = [
            "Customer_Number",
            "Account_Number",
            "TitleOfAccount",
