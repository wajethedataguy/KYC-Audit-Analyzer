...
@register_logic(
    name="064_missing_expected_local_geography",
    description="Flags customers with missing or invalid expected local geography in KYC profiles.",
    category="Compliance & Screening",
)
def logic_064_missing_expected_local_geography(
    ...
    try:
        ...
        df["EXPLOCALGEOGRAPHY_STR"] = (
            df["EXPLOCALGEOGRAPHY"].astype(str).str.strip().apply(normalize_text)
        )
        df["EXPLOCALGEOGRAPHY_NUMERIC"] = pd.to_numeric(
            df["EXPLOCALGEOGRAPHY"], errors="coerce"
        )
        ...
        df_filtered = df[
            (
                df["EXPLOCALGEOGRAPHY"].isna()
                | (df["EXPLOCALGEOGRAPHY_STR"] == "")
                | (df["EXPLOCALGEOGRAPHY_STR"].isin(nill_set_normalized))
                | (df["EXPLOCALGEOGRAPHY_NUMERIC"] == 0)
            )
        ...
        if mode == "full":
            file_path = (
                f"missing_expected_local_geography_{datetime.now():%Y%m%d_%H%M%S}.xlsx"
            )
