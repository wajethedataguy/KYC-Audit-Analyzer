...
@register_logic(
    name="056_employment_status_fed_for_non_salaried_individuals",
    description="Flags non-salaried individuals where EmploymentStatus is inappropriately fed.",
    category="Compliance & Screening",
)
def logic_056_employment_status_fed_for_non_salaried_individuals(
    ...
    try:
        ...
        df_filtered = df[
            ...
            & (df["CUSTOMER_OCCUPATION_NORMALIZED"] != "SALARIED")
            & (~df["CUSTOMER_OCCUPATION_NORMALIZED"].isin(nill_set_normalized))
        ].copy()
        ...
        # 📁 Optional export
        if mode == "full":
            file_path = f"employment_status_fed_for_non_salaried_{datetime.now():%Y%m%d_%H%M%S}.xlsx"
