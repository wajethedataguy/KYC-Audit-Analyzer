...
@register_logic(
    name="052_missing_employment_start_date_for_salaried_individuals",
    description="Flags salaried individuals with missing or vague EmployedSince dates using Nill_Comb reference list.",
    category="Compliance & Screening",
)
def logic_052_missing_employment_start_date_for_salaried_individuals(
    ...
    try:
        ...
        df["EMPLOYEDSINCE_CLEAN"] = (
            df["EMPLOYEDSINCE"].astype(str).str.strip().str.upper()
        )
        ...
        df_filtered = df[
            (
                (df["EMPLOYEDSINCE_CLEAN"] == "")
                | (df["EMPLOYEDSINCE"].isna())
                | (df["EMPLOYEDSINCE_CLEAN"].isin(nill_set))
            )
        ...
        if mode == "full":
            file_path = (
                f"missing_employment_start_date_{datetime.now():%Y%m%d_%H%M%S}.xlsx"
            )
