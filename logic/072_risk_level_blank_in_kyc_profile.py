...
@register_logic(
    name="072_risk_level_blank_in_kyc_profile",
    description="Flags customers whose KYCRisk field is blank, null, or matches known nil combinations.",
    category="Compliance & Screening",
)
def logic_072_risk_level_blank_in_kyc_profile(
    ...
    try:
        ...
        # 🔍 Normalize KYCRisk field
        df["KYCRISK_NORMALIZED"] = df["KYCRISK"].fillna("").apply(normalize_text)
        ...
        df_filtered = df[
            df["KYCRISK_NORMALIZED"].isin(nill_set_normalized)
        ...
        if mode == "full":
            file_path = (
                f"risk_level_blank_in_kyc_profile_{datetime.now():%Y%m%d_%H%M%S}.xlsx"
            )
