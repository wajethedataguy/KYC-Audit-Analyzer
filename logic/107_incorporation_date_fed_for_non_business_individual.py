...
@register_logic(
    name="107_incor_date_fed_for_non_bus_indi",
    description="Flags non-business individuals where Date of Incorporation was fed in KYC profile.",
    category="CDD & EDD Review",
)
def logic_107_incorporation_date_fed_for_non_business_individual(
    ...
    try:
        ...
        contradiction_mask = (
            ...
            & (df_kyc["CUSTOMER_OCCUPATION_CLEAN"] != "")
            & (df_kyc["CUSTOMER_OCCUPATION_CLEAN"] != "nan")
        ...
        # 📁 Optional export
        if mode == "full":
            file_path = f"incorporation_date_fed_for_non_business_individual_{datetime.now():%Y%m%d_%H%M%S}.xlsx"
        ...
        print(
            f"❌ Error in logic_107_incorporation_date_fed_for_non_business_individual: {e}"
        )
        return pd.DataFrame(
            [
                {
